...
class Apple { 
public:
    static int cnt; //статический счетчик сколько яблок

    Apple(int weight, Color_Apple color) { //конструктор принимает - вес, цвет
        /////////////////////////////двумерный вектор с объектами
        this->weight = weight; this->color = color;
        cnt++;
    }
    ~Apple() { //деструктор
        srand(1); //генерация ID начинается строго с 1
...
private:
    int weight; //вес
    Color_Apple color; //цвет
};
static int cnt = 0;

signed main() {
    Apple apple(100, RED);
    string terminal; cin >> terminal;
    
    while (terminal != "exit" || terminal != "Exit") {
        for (int i = 0; i < cnt; i++) {
            for (int j = 0; j < 2; j++) {
                if (terminal == arr[i]) {
                    /////////////////////////////доделать класс
...
INPUT: 
Apple apple1(100, RED), apple2(100, RED), apple3(150, YELLOW), apple4(500, GREEN);
__________________________________________________________________________________
OUTPUT:
ID Apple1: 9374
ID Apple2: 8254
